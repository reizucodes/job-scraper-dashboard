...
class GreenhouseScraperAdapter:
    profile_code = "greenhouse"
    ...
    def _scrape_fixture(self, source: JobSource, config: dict[str, Any]) -> list[ScrapedRecord]:
        ...
        for index, payload in enumerate(records):
            ...
            result.append(
                ScrapedRecord(
                    external_ref=str(external_ref) if external_ref is not None else f"gh-{index}",
                    raw_payload=payload,
    ...
    def _scrape_live(self, source: JobSource, config: dict[str, Any]) -> list[ScrapedRecord]:
        ...
        try:
            ...
            raise ScraperAdapterError(f"Greenhouse live fetch failed for source {source.id}: {exc}") from exc
        ...
        if isinstance(payload, dict) and isinstance(payload.get("jobs"), list):
            ...
            raise ScraperAdapterError(
                f"Greenhouse live response format invalid for source {source.id}: expected object with 'jobs' list"
            )
        ...
        for index, job in enumerate(jobs):
            ...
            result.append(
                ScrapedRecord(
                    external_ref=str(external_ref) if external_ref is not None else f"gh-live-{index}",
                    raw_payload=job,
                    source_snapshot=None,
                    scraped_at=now,
                )
    ...
    def _resolve_jobs_url(source: JobSource, config: dict[str, Any]) -> str:
        ...
        raise ScraperAdapterError(
            f"Greenhouse live mode requires 'jobs_url' in source config (source {source.id})"
        )
